  ...
  QWizardPage (parent)
{
  ...
  QLabel *label = new QLabel (tr ("The pfmExtract program is a tool for reading a PFM file and writing out a series of "
                                  "files (in ASCII, SHP, LAS, LAZ, LLZ, or RDP) for input to CARIS or other software packages.  Help is "
  ...
  if (fd->exec () == QDialog::Accepted)
    {
      ...
      if (open_args.head.proj_data.projection)
        {
          QMessageBox::warning (this, tr ("Open PFM Structure"), tr ("Sorry, pfmExtract only handles geographic PFM structures."));
          close_pfm_file (pfm_handle);
        }
      else
        {
          ...
          if (area_file_name.endsWith (".shp", Qt::CaseInsensitive))
            {
              ...
              if (shpHandle == NULL)
                {
                  QMessageBox::warning (this, "pfmExtract", tr ("Cannot open shape file %1!").arg (area_file_name));
                  return;
                }
              else
                {
                  ...
                  if (type != SHPT_POLYGON && type != SHPT_POLYGONZ && type != SHPT_POLYGONM &&
                    ...
                    {
                      QMessageBox::warning (this, "pfmExtract", tr ("Shape file %1 is not a polygon or polyline file!").arg (area_file_name));
                      return;
                    }
                  else
                    {
                      ...
                      if (shape->nVertices < 3)
                        {
                          SHPClose (shpHandle);
                          QMessageBox::warning (this, "pfmExtract", tr ("Number of vertices (%1) of shape file %2 is too few for a polygon!").arg
                          ...
                          if (shape->padfX[j] < -360.0 || shape->padfX[j] > 360.0 || shape->padfY[j] < -90.0 || shape->padfY[j] > 90.0)
                            {
                              ...
                              SHPClose (shpHandle);
                              QMessageBox::warning (this, "pfmExtract", tr ("Shape file %1 does not appear to be geographic!").arg (area_file_name));
                              return;
                            }
                        }
